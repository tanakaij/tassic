...
// Strategy: NETWORK-FIRST with a deadline, cache as the fallback.
//
// The previous version served navigations and static assets cache-first and
// refreshed them in the background. For a Kotlin/Wasm app that is a trap: the
// bundle is several independently hashed files (composeApp.js, the
// uninstantiated .mjs, the .wasm, skiko.*), and background refresh replaces
// them one at a time. After a deploy a page could be handed a NEW loader with
// an OLD wasm (or the reverse); the mismatch fails during instantiation and
// the result is a permanently blank screen that only clearing site data fixes.
// Going to the network first means the files a page gets always come from one
// deploy; the cache is only what makes offline launches work.
const NETWORK_DEADLINE_MS = 6000;

async function cachePut(req, res) {
    // Only complete, successful, same-origin responses. A 206 partial (range
    // request) or an opaque/error response would poison the cache.
    if (!res || res.status !== 200 || res.type === 'opaque') return;
    try {
        const cache = await caches.open(CACHE);
        await cache.put(req, res.clone());
    } catch (e) { /* quota or transient — the network copy still works */ }
}

async function networkFirst(event, req, cacheKey) {
    try {
        const res = await fetchWithTimeout(req, NETWORK_DEADLINE_MS);
        if (res && res.status === 200) event.waitUntil(cachePut(cacheKey || req, res));
        return res;
    } catch (e) {
...
self.addEventListener('fetch', (event) => {
        ...
        event.respondWith(
            (async () => {
                try {
                    ...
                    const res = preloaded || await fetchWithTimeout(req, NETWORK_DEADLINE_MS);
                    if (res && res.status === 200) event.waitUntil(cachePut('./index.html', res));
                    return res;
                } catch (e) {
                    const cached = await caches.match('./index.html') || await caches.match('./');
                    return cached || OFFLINE_FALLBACK.clone();
                }
            })()
        );
        return;
    ...
    // Static assets (js / mjs / wasm / css / images / fonts): network first.
    event.respondWith(
        networkFirst(event, req).catch(() => Response.error())
    );
});
