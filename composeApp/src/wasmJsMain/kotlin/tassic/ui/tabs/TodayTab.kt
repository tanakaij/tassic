...
import tassic.ui.components.ConfirmAction
...
fun TodayTab(onOpenTab: (Tab) -> Unit = {}) {
    ...
    val trackedHabits by store.habits.items.collectAsState()
    ...
    var resetConfirmFor by rememberState<RecoveryHabit?>(null)
    ...
    // mod(0) throws, and the lists are empty on a fresh install (nothing is
    // seeded until first-run finishes) or when the Music module is off. That
    // exception killed the very first composition and left a blank screen.
    val mode = if (modes.isEmpty()) null else modes[(T.dayOfYear(today) - 1).mod(modes.size)]
    val keys = practice.filter { it.kind == PracticeKind.KEY }.sortedBy { it.sortOrder }
    val key = if (keys.isEmpty()) null else keys[T.dayOfYear(today).mod(keys.size)]
    ...
    TabScaffold(
        ...
        if (view == "Habits") {
            val pulses = remember(trackedHabits, activity, today) { Coach.allPulses(store, today) }
            TassicCard {
                SectionHeader(
                    title = "Habits",
                ...
                if (pulses.isEmpty()) {
                    ...
                    pulses.forEachIndexed { index, pulse ->
                        HabitRow(
                            ...
                            onDelete = { store.deleteHabit(pulse.habit.id) }
                        )
        ...
        habits.filter { it.active }.forEach { h ->
            TassicCard {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.padding(top = 10.dp)) {
                    ...
                    GhostButton("Reset counter", { resetConfirmFor = h })
                }
            }
        }
        ...
        resetConfirmFor?.let { h ->
            ConfirmAction(
                title = "Reset ${h.name}?",
                message = "Today's run resets to day 1. It's folded into Best first, so the streak itself isn't lost.",
                confirmLabel = "Reset",
                onConfirm = { store.resetRecoveryCounter(h, today) },
                onDismiss = { resetConfirmFor = null }
            )
        }
