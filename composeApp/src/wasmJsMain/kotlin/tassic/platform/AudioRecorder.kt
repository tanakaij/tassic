...
private fun jsIsSupported(): Boolean = js(
...
private fun jsPickMime(): JsString? = js(
...
private fun jsStart(mime: String): Promise<JsAny?> = js(
    """(async () => {
...
private fun jsStop(controller: JsAny): Promise<JsString?> = js("controller.stop()")

private fun jsPlay(dataUrl: String): JsAny? = js("(()=>{ try { const el = new Audio(dataUrl); el.play(); return el; } catch (e) { return null; } })()")

private fun jsPause(player: JsAny): Unit = js("(()=>{ try { player.pause(); } catch (e) {} })()")
...
object AudioRecorder {
     ...
     */
    fun start(mime: String): Promise<JsAny?> = jsStart(mime)

    fun stop(controller: JsAny): Promise<JsString?> = jsStop(controller)

    /** Plays a data: URL clip; returns the HTMLAudioElement so it can be paused later. */
    fun play(dataUrl: String): JsAny? = jsPlay(dataUrl)

    fun pause(player: JsAny): Unit = jsPause(player)
}
