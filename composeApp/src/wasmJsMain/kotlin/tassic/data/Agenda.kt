    ...
    fun plan(store: Store, day: Long = T.today(), nowMinutes: Int = T.localMinuteOfDay()): DayPlan {
        ...
        store.activeHabits().forEach { habit ->
            ...
            entries += AgendaEntry(
                ...
                // A set reminder hour is the time the user actually chose,
                // so it takes priority over the coarse ANY/MORNING/AFTERNOON
                // bucket - otherwise the schedule shows a different time than
                // the one picked in the habit editor.
                startMinutes = if (habit.reminderOn) habit.reminderHour * 60 else slotForTimeOfDay(habit.timeOfDay),
                durationMinutes = 15,
                done = count >= target,
                refId = habit.id,
                accent = habit.color
