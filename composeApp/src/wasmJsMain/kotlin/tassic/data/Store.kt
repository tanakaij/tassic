...
class Store() {
    ...
    /**
     * Restarts the counter without logging a relapse (e.g. correcting a
     * mistaken start date). Still folds the run being ended into bestStreak,
     * same as a relapse would, so a manual reset can never erase a streak
     * that was actually longer than what's on record.
     */
    fun resetRecoveryCounter(habit: RecoveryHabit, today: Long = T.today()) {
    ...
    /**
     * Adds one rep, capped at the daily target. Once "8 glasses" reads 8/8
     * the tile shows a checkmark instead of a number, so further taps must
     * be a no-op rather than silently padding the day's count with reps
     * nobody can see are still being recorded.
     */
    fun tickHabit(habit: Habit) {
        val today = T.today()
        val target = habit.targetPerDay.coerceAtLeast(1)
        if (habitCount(habit.id, today) >= target) return
     ...
     */
    /**
     * Records that you spoke to someone, on the given day.
     *
     * Defaults to today, but takes an explicit day so a conversation that
     * actually happened last week and was simply forgotten can be logged
     * against the day it happened rather than falsely inflating today's
     * activity and cadence countdown.
     */
    fun logContact(person: Person, note: String = "", epochDay: Long = T.today()) {
        replace(
            people,
            person.copy(
                // The day is whatever the user picked — today, yesterday, a
                // week ago, or a custom date. Clamping this to the max of
                // the old and new value silently ignored any day earlier
                // than what was already on file, which is exactly the
                // "log it for the day it actually happened" case this
                // function exists for. Take the chosen day as-is.
                lastContactEpochDay = epochDay,
                notes = if (note.isBlank()) person.notes else (note.trim() + "\n" + person.notes).trim()
            )
        )
        logActivity("people", person.name, person.id, event = "CONTACT", epochDay = epochDay)
    }
    ...
    fun startReadingPlan(template: Bible.PlanTemplate): ReadingPlan {
        ...
        // A plan with zero days read is just an abandoned choice, not
        // history worth keeping, so it's dropped outright rather than left
        // to pile up silently every time someone changes their mind.
        val (empty, started) = readingPlans.flow.value.partition { it.completedDays.isEmpty() }
        empty.forEach { remove(readingPlans, it.id) }
        readingPlans.flow.value = started.map { it.copy(active = false) }
